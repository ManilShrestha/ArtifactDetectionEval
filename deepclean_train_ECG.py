...
def train(train_files, test_file, reset=False):
	...
	if reset:
		...
		torch.save(mean, f'{config["stored_mean_ecg"]}_{segment_length_sec}sec')
		torch.save(std, f'{config["stored_std_ecg"]}_{segment_length_sec}sec')
	else:
...
def test(test_file, vae=None):
	...
	Args:
		test_file (str): a single test file used to test the model on
		vae: model passed, defaults to None, if None, then load the best model
	"""
	if vae is None:
	...
	with torch.no_grad():
		for _, data, label in dataloader:
			...
			# Assuming the shape of `data` and `recon_batch` is [batch_size, channels, height, width]
			# `.mean([1,2,3])` computes the mean across channels, height, and width, resulting in a single loss value per instance
