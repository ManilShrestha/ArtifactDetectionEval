...
def train(train_files, reset=False):
...
def test(test_file, train_files, vae=None):
	...
	return confusion_matrix(truth_label, pred_label)
...
if __name__ == '__main__':
	...
	# Iterate through each file to use it as the training set
	for train_file in files:
		train_files = [train_file]
		print(f"Training File: {train_file}")
		model = train(train_files, reset=True)
		
		# Use each of the other files as a test file
		for test_file in files:
			print("Testing Files:", test_file)
			row_t = []
			cm = test(test_file, train_files, model)

			tn, fp, fn, tp = cm[0][0], cm[0][1],cm[1][0],cm[1][1]

			row_t = [len(train_files), "|".join(train_files), test_file, tn, fp, fn, tp]
			results.append(row_t)
	
	# Open the file in write mode
	with open('backup/train-1-test-1.csv', 'w', newline='') as file:
		writer = csv.writer(file)
		
		# Write each list (row) into the CSV
		for row in results:
			writer.writerow(row)
